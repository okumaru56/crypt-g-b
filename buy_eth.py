...
def main():
    ...
    try:
        ...
        if jpy_free < jpy_budget:
            ...
            send_email(f"【失敗】ETH自動積立通知 ({date_str})", f"ETHの自動買付に失敗しました。\n\n理由:\n{error_msg}")
            sys.exit(1)
        ...
        # GMOコインのETH現物取引最小単位は 0.01 ETH（小数第2位で切り捨て）
        raw_amount = jpy_budget / last_price
        buy_amount = math.floor(raw_amount * 100) / 100

        if buy_amount < 0.01:
            error_msg = f"購入計算数量({buy_amount} ETH)が最小取引単位(0.01 ETH)未満です。"
            send_email(f"【失敗】ETH自動積立通知 ({date_str})", f"ETHの自動買付に失敗しました。\n\n理由:\n{error_msg}")
            return
        ...
        success_body = (
            f"GMOコインでのETH自動買付が正常に完了しました。\n\n"
            f"----------------------------------------\n"
            f"実行日時: {date_str}\n"
            f"注文ID  : {order_id}\n"
            f"購入金額: 約 {int(buy_amount * last_price):,} 円 ({jpy_budget:,} 円設定)\n"
            f"購入数量: {buy_amount} ETH\n"
            f"適用レート目安: {int(last_price):,} JPY/ETH\n"
            f"----------------------------------------"
        )
        print(success_body)
        send_email(f"【成功】ETH自動積立完了通知 ({date_str})", success_body)

    except Exception as e:
        error_detail = f"処理中に例外が発生しました:\n{str(e)}"
        print(f"エラー: {error_detail}")
        send_email(f"【失敗】ETH自動積立システムエラー ({date_str})", error_detail)
        sys.exit(1)
